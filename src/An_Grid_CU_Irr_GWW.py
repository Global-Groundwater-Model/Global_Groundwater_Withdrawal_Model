...
# -*- coding: utf-8 -*-
"""
Created on Thu May 16 10:26:43 2024

@author: sara.nazari
"""

## ----------------------------------------------------------------------------------------- ##
#    Calculate annual countries Per Grid Consumptive Irrigation groundwater withdrawal       #
#                           Consider the Irrigation Efficieny                                #
...
def CU_Grid_Irr_GWW (fn_assigned_countries_Irr_Grid_Irr_GWW_F_Irr, fn_GW_Irr_Area, start_year, end_year,fn_An_Grid_CU_Irr_GWW):
    
    raster = rasterio.open(fn_GW_Irr_Area)
    vector = gpd.read_file(fn_assigned_countries_Irr_Grid_Irr_GWW_F_Irr, crs="epsg:4326")
    ...
    for year in range(start_year, end_year+1):    
        geom_value = ((geom,value) for geom, value in zip(vector.geometry, vector['IrrGWG'+str(year)]*vector['F_Irr']))
    ...
    ar_An_Irr_Area_Grid_Irr_GWW = xr.DataArray(An_Irr_Area_Grid_Irr_GWW, dims = ['time','latitude','longitude'],coords =[df_time.Date[:], latitude, longitude], 
                                    attrs=dict(description="Calculation of per irrigated area's grid net irrigation groundwater withdrawal (Considering Groundwater Irrigation Efficiency) for the country", units='m3/year')) 
    ds_Irr_Area_Grid_Irr_GWW = ar_An_Irr_Area_Grid_Irr_GWW.to_dataset(name='T_CU_Irr_GWW_Irr_Areas')
    
    ds_Irr_Area_Grid_Irr_GWW.to_netcdf(fn_An_Grid_CU_Irr_GWW)

CU_Grid_Irr_GWW (snakemake.input[0],snakemake.input[1], snakemake.params[0], snakemake.params[1], snakemake.output[0])